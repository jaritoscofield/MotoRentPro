...
@section('title', 'Nova Venda - MotoRentPro')
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Nova Venda</h2>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Cadastre uma nova venda ou aluguel</p>
                </div>
        ...
        <form method="POST" action="/pagamentos" class="p-6">
            @csrf
                    ...
                    <div>
                        ...
                        <select name="sale_type" id="sale_type" required 
                            ...
                            @foreach($saleTypes as $value => $label)
                                <option value="{{ $value }}" {{ old('sale_type') == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="periodicity" id="periodicity" required 
                            ...
                            @foreach($periodicities as $value => $label)
                                <option value="{{ $value }}" {{ old('periodicity') == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="mt-6">
                ...
                <textarea name="notes" id="notes" rows="3"
                          class="w-full px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-md text-xs focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400"
                          placeholder="Observações adicionais sobre a venda...">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
