...
@section('title', 'Editar Reserva - MotoRentPro')
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Editar Reserva</h1>
                    <p class="text-gray-600 mt-1">Atualize as informações da reserva</p>
                </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm">
            <form method="POST" action="/reservas/{{ $reservation->id }}" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="client_id" 
                                name="client_id"
                                class="block w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500 @error('client_id') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <select id="motorcycle_id" 
                                name="motorcycle_id"
                                class="block w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500 @error('motorcycle_id') border-red-500 @enderror">
                            <option value="">Selecione uma motocicleta</option>
                            @foreach($motorcycles as $motorcycle)
                                <option value="{{ $motorcycle->id }}" 
                                        data-daily-rate="{{ $motorcycle->daily_rate }}"
                                        {{ old('motorcycle_id', $reservation->motorcycle_id) == $motorcycle->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="date" 
                               id="start_date" 
                               name="start_date" 
                               value="{{ old('start_date', $reservation->start_date ? $reservation->start_date->format('Y-m-d') : '') }}"
                               class="block w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500 @error('start_date') border-red-500 @enderror">
                        @error('start_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" 
                               ...
                               class="block w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500 @error('end_date') border-red-500 @enderror">
                        @error('end_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Taxa Diária -->
                    <div>
                        <label for="daily_rate" class="block text-sm font-medium text-gray-700 mb-1">Taxa Diária (R$) *</label>
                        <input type="number" 
                               id="daily_rate" 
                               name="daily_rate" 
                               value="{{ old('daily_rate', $reservation->daily_rate) }}"
                               step="0.01"
                               min="0"
                               class="block w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500 @error('daily_rate') border-red-500 @enderror"
                               placeholder="0.00">
                        @error('daily_rate')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select id="status" 
                            ...
                            @foreach($statuses as $status)
                                <option value="{{ $status }}" {{ old('status', $reservation->status) == $status ? 'selected' : '' }}>{{ $status }}</option>
                            @endforeach
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="md:col-span-2">
                        <div class="bg-gray-50 rounded-lg p-4">
                            <h3 class="text-sm font-medium text-gray-900 mb-2">Cálculo Automático</h3>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    <p class="text-xs text-gray-500">Duração</p>
                                    <p id="duration" class="text-sm font-medium text-gray-900">{{ $reservation->duration_days }} dia(s)</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500">Taxa Diária</p>
                                    <p id="daily_rate_display" class="text-sm font-medium text-gray-900">{{ $reservation->formatted_daily_rate }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500">Valor Total</p>
                                    <p id="total_amount" class="text-lg font-bold text-green-600">{{ $reservation->formatted_total_amount }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="notes" 
                                  ...
                                  placeholder="Informações adicionais sobre a reserva">{{ old('notes', $reservation->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-200">
                    <a href="/reservas" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                        Cancelar
                    </a>
